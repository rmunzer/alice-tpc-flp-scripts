...
do 
	...
	if [[ $command == "cru_config_force" ]]; then 
		ssh tpc@alio2-cr1-flp$i "source ./scripts/cru_config_force.sh" & 
	fi
	if [[ $command == "restart_alf" ]]; then ssh tpc@alio2-cr1-flp$i "source ./scripts/restart_alf.sh"; fi
	if [[ $command == "rescan" ]]; then ssh tpc@alio2-cr1-flp$i "sudo ./scripts/rescan.sh 1"; fi
	if [[ $command == "restart_alf_force" ]]; then ssh tpc@alio2-cr1-flp$i "source ./scripts/restart_alf.sh 1"; fi
