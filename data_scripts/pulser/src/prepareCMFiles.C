...
void prepareCMFiles(const std::string_view pulserFile, std::string outputDir = "./")
{
  ...
  if (pulserFile.find("cdb") != std::string::npos) {
    ...
    pulserT0 = &pulserData.at("T0");
  } else {
    auto calPads = utils::readCalPads(pulserFile, "Qtot,T0");
  ...
  for (size_t iroc = 0; iroc < pulserQtot->getData().size(); ++iroc) {
    ...
    const auto& rocPulserT0 = pulserT0->getCalArray(iroc);
    ...
    for (size_t ipad = 0; ipad < rocPulserQtot.getData().size(); ++ipad) {
      ...
      float pulserVal2 = rocPulserT0.getValue(ipad);
      ...
      if (fabs(pulserVal2-538) > 4) {
        LOGP(error, "Wrong Pulser T0 value in ROC {:2}, CRU {:3}, fec in CRU: {:2}, SAMPA: {}, channel: {:2}, pulserT0: {:.4f}", iroc, cruID, fecInPartition, sampa, sampaChannel, pulserVal2);
      }
